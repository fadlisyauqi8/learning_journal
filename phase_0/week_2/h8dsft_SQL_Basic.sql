...
-- Melihat dosen dengan gaji tertinggi di Stanford University
SELECT first_name, last_name, school, salary
FROM teachers
WHERE school='Stanford University' MAX(salary);


-- Menampilkan query para dosen dengan no identitas,jumlah matkul, jumlah siswa & jumlah dosen per matkul
SELECT * 
FROM teachers
JOIN courses ON teachers.id=courses.teachers_id
WHERE (courses.name, teachers.salary) IN (

  SELECT courses.name,teachers.salary
  FROM teachers
  JOIN courses ON teachers.id=courses.teachers_id
